...
class Hyper:
    ...
    def __init__(self, captcha_type:CaptchaType=CaptchaType.SUPREME_COURT, weights_only=True, quiet_out=False):
        ...
        self.train_image_files = self.get_image_files(captcha_type, True)
        self.pred_image_files = self.get_image_files(captcha_type, False)
        self.model_path = self.get_model_path(captcha_type)
        self.image_width, self.image_height, self.max_length, self.characters = self.get_train_info()
    ...
    def get_train_info(self):
        img_path = self.train_image_files[-1]
        img = Image.open(img_path)
        img_width = img.width
        img_height = img.height
        labels = [train_image_file.split(os.path.sep)[-1].split(".png")[0] for train_image_file in self.train_image_files]
        # labels = [train_image_file.split(os.path.sep)[-1].split(".png")[0] for train_image_file in self.train_image_files]
        max_length = max([len(label) for label in labels])
        characters = sorted(set(char for label in labels for char in label))
        return img_width, img_height, max_length, characters

    def get_model_path(self, cap_type:CaptchaType, weightsOnly=True):
        baseDir = self.get_base_dir()
    ...
    def get_model(self, captcha_type:CaptchaType=None, weight_only=None):
        if captcha_type == None:
            captcha_type = self.captcha_type
        if weight_only == None:
            weight_only = self.weights_only
        ...
        weights_path = self.get_model_path(captcha_type, weight_only)
    ...
    def model_train(self, captcha_type:CaptchaType, patience:int=7):
        ...
        weights_path = self.get_model_path(captcha_type, True)
        model.save_weights(weights_path)
        weights_path = self.get_model_path(captcha_type, False)
        model.save(weights_path)

    def model_validate(self, captcha_type:CaptchaType=None, weight_only=None):
        start = time.time()
        matched = 0
        ...
        # max_length, characters = self.get_train_info(train_img_path_list)
        # weights_path = self.get_model_path(captcha_type, weight_only)
        model = ApplyModel(self.model_path, self.image_width, self.image_height, self.max_length, self.characters)

        for pred_img_path in self.pred_img_path_list:
        ...
        print("Matched:", matched, ", Tottal : ", len(self.pred_image_files))
...
#         self.weights_only = weights_only
#         self.hyper = Hyper()
#         self.weights_path = self.hyper.get_model_path(captcha_type, weights_only)
